...
pub fn emit_executable_tjs(file: &Tjs2File) -> Result<String> {
    ...
    out.push_str(
        "// This is a low-level, semantics-oriented re-materialization of TJS2 bytecode.\n",
    );
    ...
    for obj in &file.objects {
        ...
        out.push('\n');
        out.push_str(&format!(
            "__tjs2dec_objs[{idx}] = __tjs2dec_obj_{idx};\n\n",
            idx = obj.index
        ));
    }
...
fn emit_object(obj: &Tjs2Object, pools: &ConstPools) -> Result<String> {
    ...
    // Registers.
    let maxv = if obj.max_variable_count < 0 {
        0
    } else {
        obj.max_variable_count
    };
...
fn emit_op2_prop(
    ...
    match kind {
        ...
        1 => {
            ...
            out.push_str(&format!(
                "          __tjs2dec_set_prop({}, __k, __t, false);\n",
                r(objr)
            ));
            if dst != 0 {
                out.push_str(&format!(
...
",
        ...
        _ => {
            out.push_str(&format!(
                "          throw \"Unimplemented opcode: {} (unknown op2_prop kind) at ip={}\n\";
",
                code[pc], pc
            ));
        }
...
fn emit_case(
    ...
    match op {
        ...
        x if x >= vm::VM_INC && x <= vm::VM_INCP => match x - vm::VM_INC {
            0 => {
                ensure(code, pc, 2)?;
                let rr = code[pc + 1];
...
",
        ...
        x if x >= vm::VM_DEC && x <= vm::VM_DECP => match x - vm::VM_DEC {
            0 => {
                ensure(code, pc, 2)?;
                let rr = code[pc + 1];
                out.push_str(&format!("          {} = ({} - 1);\n", r(rr), r(rr)));
...
",
        ...
        x if x >= vm::VM_LOR && x <= vm::VM_LORP => match x - vm::VM_LOR {
            0 => {
                ensure(code, pc, 3)?;
                let a = code[pc + 1];
                let b = code[pc + 2];
...
",
        ...
        x if x >= vm::VM_LAND && x <= vm::VM_LANDP => match x - vm::VM_LAND {
            0 => {
                ensure(code, pc, 3)?;
                let a = code[pc + 1];
                let b = code[pc + 2];
            ...
            if argc == -2 {
                ...
                for j in 0..num {
                    ...
                    match ty {
                        ...
                        FAT_EXPAND => out.push_str(&format!(
                            "          __tjs2dec_expand_args(__args, {});\n",
                            r(v)
                        )),
                        FAT_UNNAMED_EXPAND => {
...
fn variant_to_tjs(v: &Variant, pools: &ConstPools, _obj_index: usize) -> String {
    match v {
        ...
        Variant::Real(i) => pools
            .doubles
            .get(*i as usize)
            .copied()
            .unwrap_or(0.0)
            .to_string(),
        Variant::Byte(i) => pools
            .bytes
            .get(*i as usize)
            .copied()
            .unwrap_or(0)
            .to_string(),
        Variant::Short(i) => pools
            .shorts
            .get(*i as usize)
            .copied()
            .unwrap_or(0)
            .to_string(),
        Variant::Integer(i) => pools
            .ints
            .get(*i as usize)
            .copied()
            .unwrap_or(0)
            .to_string(),
        Variant::Long(i) => pools
            .longs
            .get(*i as usize)
            .copied()
            .unwrap_or(0)
